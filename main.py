...
#model predict 7 topic for each paper
#dictionary checks which label belongs to which topic
CORA_CLASSES = {
    0: "Case_Based",
...
#show basic details about the model inout,output,class
@app.get('/info')
def model_info():
    return {
        "Model Name": "SimpeGCN",
...
#Classify a graph you provide yourself and optionally your own edges/connected
@app.post("/predict")
def predict_custom_graph(request: GraphPredictRequest):
    ...
    if request.edge_indices:
        ...
        #not get edge indeces from user then connect evry node itself
    else:
...
#classify Real Papers from actual CORA dataset
...
def predict_real_cora_nodes(request: CoraNodeRequest):
    ...
    if invalid_index:
        raise HTTPException(
            400,
            f"node_index out of bound (must be 0 to {largest_valid_index})"
        )
